...
- 当前实验只在 Windows 11 25H2（build 26200）验证，通知实际点击、冷启动、系统设置关闭和全新无运行时机器尚未完成真实桌面人工验收；独立 Windows 10 环境仍未验证，不能据此声称跨版本或生产稳定性。原始注册表 `ProductName=Windows 10 Pro` 与 build/25H2 不一致，整合记录按 build 26200 归类为 Windows 11 25H2。
